...
openssl req -x509 -nodes -days 9999 -newkey rsa:2048 -keyout azurite.key -out azurite.crt -config azurite.conf -extensions v3_req

# 2. Install to system trust store (Linux):
sudo cp ./azurite.crt /usr/local/share/ca-certificates/azurite.crt
sudo update-ca-certificates

# 3. Install to Chrome's certificate database (NSS):
sudo apt update && sudo apt install -y libnss3-tools
certutil -d sql:$HOME/.pki/nssdb -A -t "CP,CP," -n "azurite" -i ./azurite.crt
